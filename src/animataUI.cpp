...
void AnimataUI::cb_tabs_i(Fl_Tabs* o, void*) {
    /* callback to reset the active button when the tab is selected again */
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        if (b->value() && (b != playback_show_hide))
        {
            /* don't call the callback of view/show, because it makes the output
             * window flash */
            b->do_callback();
        }
    }
...
void AnimataUI::cb_Create_i(Fl_Button*, long v) {
    ...
    /* save previous mode to detect mode changes */
